...
def makehappytiff(dst_path, seams_path):
    ...
    imsize = 1065
    testdata = [
        (np.random.rand(imsize, imsize) * 255).astype(np.uint8) for i in range(4)
    ]
    ...
    if seams_path:
        ...
        rInds = np.arange(frto[0], frto[1], (frto[1] - frto[0]) / float(imsize)).astype(
            int
        )
...
def getnulldiff(in1, in2, threshold):
    ...
    assert diff >= threshold, "input 1 has more than %d nodata pixels than input 2" % (
        threshold
    )
